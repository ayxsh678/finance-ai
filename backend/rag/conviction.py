...
_conv_cache: TTLCache = TTLCache(maxsize=100, ttl=3600)  # 1 hour cache
...
def _score_52w_position(price, low52, high52):
    ...
    if 55 <= pct <= 80:
        return 10, f"{pct:.0f}% of 52W range"
    if 40 <= pct < 55:
        return 8, f"{pct:.0f}% of 52W range"
    if 80 < pct <= 90:
        return 7, f"{pct:.0f}% of 52W range"
    if 25 <= pct < 40:
        return 5, f"{pct:.0f}% of 52W range"
    if pct > 90:
        return 5, f"{pct:.0f}% of 52W range"
...
def _score_rel_volume(rel_vol, price_change):
    ...
    if v > 2.0:
        return (10 if up else 2), f"{v:.2f}× avg"
    if v > 1.5:
        return (8 if up else 3), f"{v:.2f}× avg"
    if v > 0.8:
        return 5, f"{v:.2f}× avg"
...
def _score_rsi(rsi):
    ...
    if 30 <= rsi < 40:
        return 5, f"{rsi:.0f}"
    if 70 < rsi <= 80:
        return 3, f"{rsi:.0f} (overbought)"
    if rsi > 80:
        return 1, f"{rsi:.0f} (overbought)"
...
def _score_sma20(price, sma20, prev_sma20):
    ...
    if above and rising:
        return 10, f"{sma20:.1f} (above ↑)"
    if above:
        return 7, f"{sma20:.1f} (above)"
    if not above and not rising:
        return 1, f"{sma20:.1f} (below ↓)"
...
def _score_trend_consistency(closes):
    ...
    if up_days >= 14:
        return 10, f"{up_days}/19 up days"
    if up_days >= 11:
        return 8, f"{up_days}/19 up days"
    if up_days >= 9:
        return 6, f"{up_days}/19 up days"
    if up_days >= 7:
        return 4, f"{up_days}/19 up days"
    if up_days >= 5:
        return 2, f"{up_days}/19 up days"
...
def _score_pe(pe):
    ...
    if v <= 0:
        return 5, "N/A"
    if v < 12:
        return 9, f"PE {v:.1f} (deep value)"
    if v < 18:
        return 10, f"PE {v:.1f} (fair)"
    if v < 28:
        return 7, f"PE {v:.1f} (market rate)"
    if v < 40:
        return 4, f"PE {v:.1f} (growth premium)"
    if v < 60:
        return 2, f"PE {v:.1f} (expensive)"
...
def _score_earnings_trajectory(trailing_pe, forward_pe):
    ...
    if ratio < 0.80:
        return 10, f"FWD {f:.1f} vs TTM {t:.1f}"
    if ratio < 0.95:
        return 8, f"FWD {f:.1f} vs TTM {t:.1f}"
    if ratio < 1.05:
        return 5, f"FWD {f:.1f} vs TTM {t:.1f}"
    if ratio < 1.20:
        return 3, f"FWD {f:.1f} vs TTM {t:.1f}"

    return 1, f"FWD {f:.1f} vs TTM {t:.1f}"
...
def compute_conviction(ticker: str, company_name: str = "") -> dict:
    ...
    try:
        ...
        try:
            ...
            logger.warning("[conviction] yfinance history error for %s: %s", ticker, e)
